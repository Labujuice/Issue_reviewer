...
def main():
    parser = argparse.ArgumentParser(description="GitHub Issue Analyzer Agent")
    parser.add_argument("--days", type=int, help="只分析過去 N 天更新的 Issues")
    parser.add_argument("--issue-id", type=int, help="指定分析單一 Issue ID (Number)")
    ...
    parser.add_argument("--mock", action="store_true", help="使用 Mock 測試數據（無需連接 GitHub）")
    
    args = parser.parse_args()

    logger.info("========================================")
    logger.info("   GitHub Issue Analyzer Agent 啟動")
    logger.info("========================================")
    ...
    if args.mock:
        ...
        logger.info("模式: [連接 GitHub API]")
        try:
            Config.validate()
        except ValueError as e:
            logger.error(str(e))
            logger.error("請確認您的環境變數或 .env 檔案設定是否完整！或使用 --mock 參數進行測試。")
            sys.exit(1)
        ...
        try:
            if args.issue_id:
                logger.info(f"正在單獨抓取 Issue #{args.issue_id}...")
                # Fetch single issue via requests
                headers = {"Accept": "application/vnd.github+json"}
                if fetcher.token:
                    headers["Authorization"] = f"token {fetcher.token}"
                url = f"{fetcher.api_url}/repos/{fetcher.repo}/issues/{args.issue_id}"
                
                response = fetcher.gl_request = requests.get(url, headers=headers)
                ...
                logger.info("正在抓取專案的 Open Issues...")
    ...
    if args.slack:
        if Config.SLACK_WEBHOOK_URL:
            ...
            summary_msg = f"🤖 *GitHub Issue 分析每日報告出來囉!*\n" \
    ...
    if args.discord:
        if Config.DISCORD_WEBHOOK_URL:
            ...
            summary_msg = f"🤖 **GitHub Issue 分析每日報告已產出**\n" \
                          f"共分析了 **{len(analyses)}** 個 Issues。\n" \
